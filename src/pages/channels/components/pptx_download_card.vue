  ...
  <div class="w-full" v-if="mol_data">
    <Card class="w-full overflow-hidden">
      <div class="flex flex-row items-center justify-between p-4">
        <div class="flex-grow">
          <h4 class="text-sm font-semibold text-primary mb-1">PPTX结果下载</h4>
          <p class="text-sm text-muted-foreground">点击右侧按钮下载PPTX结果</p>
        </div>
...
const mol_data = computed(() => {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    if (item.tool_name.indexOf('PPT') != -1 && item.description && item?.description?.key) {
      key.value = item?.description?.key
      // 能否通过url直接下载
    }
  }
  return key.value
